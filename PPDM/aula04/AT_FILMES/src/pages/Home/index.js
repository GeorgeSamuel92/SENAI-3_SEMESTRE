...
export default function App() {
  ...
  const [allNofications, setAllNotifications] = useState([]);
  ...
  useEffect(() => {
    if (notificationReceived !== null) {
      ...
      const {
        date,
        request: { content, identifier, triger },
      } = notificationReceived;

      dados = {
        date: date,
        bodyMenssage: constent.body,
        titleMenssage: content.title,
      };
  ...
  useEffect(() => {
      if (Platform.OS === "android") {
        console.log('All:', allNofications);
      }
    }, [allNofications]);
    ...
    <SafeAreaProvider>
      <SafeAreaView style={styles.androidSafeArea}>
        <View style={styles.container}>
          ...
          <TouchableOpacity style={styles.button} onPress={allFilms}>
            <Text style={styles.textButton}>Acessar</Text>
          </TouchableOpacity>
        </View>
